            ...
            for (int x = 0; x < world.WorldWidth; x++)
            {
                ...
                for (int y = 0; y < world.WorldHeight; y++)
                {
                    if (y <= groundLevel)
                    {
						...
						SetBlock(x, y + 1, (byte)Layers.Main, (byte)MainLayer.PurpleRock);
                        SetBlock(x, y, (byte)Layers.Main, (byte)MainLayer.GreenRock, 20);
						if (PerlinNoise(x * 3, y, 20, 3, 1) > 1)
                        {
                        ...
                        if (y == groundLevel && IsBlockAt(x, y, (byte)Layers.Main))
                        {
                            ...
                            if (DoAddBlock(40))
                            {
                                ...
                                if (genTree)
                                {
                                    ...
                                    SetBlock(x - 1, y + 2, (byte)Layers.Tree, (byte)TreeLayer.Stump);
